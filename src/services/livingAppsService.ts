...
import type { Schichttypen, Mitarbeiter, Schichtplan, CreateSchichttypen, CreateMitarbeiter, CreateSchichtplan } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getSchichtplanEntry(id: string): Promise<Schichtplan | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Schichtplan;
    return enrichLookupFields([record], 'schichtplan')[0];
  }
  static async createSchichtplanEntry(fields: CreateSchichtplan) {
    return callApi('POST', `/apps/${APP_IDS.SCHICHTPLAN}/records`, { fields: cleanFieldsForApi(fields as any, 'schichtplan') });
  }
  static async updateSchichtplanEntry(id: string, fields: Partial<CreateSchichtplan>) {
    return callApi('PATCH', `/apps/${APP_IDS.SCHICHTPLAN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schichtplan') });
  }
